...
const ComicDetailPage = () => {
  ...
  if (!comic) {
    return (
      <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 py-20 text-center">
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 py-6 sm:py-8">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-sm text-wabi-muted mb-4 sm:mb-6 flex-wrap" aria-label="breadcrumb">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 mb-12 sm:mb-16">
        ...
        <div className="space-y-3">
          <div className="rounded-2xl overflow-hidden paper border border-wabi-border">
            <Swiper modules={[Navigation, Pagination, Thumbs]} navigation pagination={{ clickable: true }} thumbs={{ swiper: thumbsSwiper && !thumbsSwiper.destroyed ? thumbsSwiper : null }} loop={images.length > 1} className="comic-detail-swiper" id="comic-main-swiper">
              {images.map((img, idx) => (
                <SwiperSlide key={idx}>
                  <img src={img} alt={`${comic.title} - ảnh ${idx + 1}`} className="w-full max-h-[400px] sm:max-h-[560px] object-contain bg-wabi-bg2" onError={(e) => { e.target.src = 'https://placehold.co/400x560/e8ddd1/3d2b1a?text=No+Image' }} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
          {images.length > 1 && (
            <Swiper modules={[Thumbs]} onSwiper={setThumbsSwiper} spaceBetween={8} slidesPerView={Math.min(images.length, 5)} watchSlidesProgress className="cursor-pointer" id="comic-thumb-swiper">
              {images.map((img, idx) => (
                <SwiperSlide key={idx}>
                  <img src={img} alt={`thumb ${idx + 1}`} className="w-full aspect-[2/3] object-cover rounded-lg border-2 border-transparent hover:border-wabi-red transition-all" onError={(e) => { e.target.src = 'https://placehold.co/100x150/e8ddd1/3d2b1a?text=' + (idx + 1) }} />
                </SwiperSlide>
              ))}
            </Swiper>
          )}
        ...
        <div className="flex flex-col gap-3 sm:gap-4 animate-fade-up">
          ...
          <h1 className="font-serif text-xl sm:text-2xl md:text-3xl font-bold text-wabi-text leading-tight">{comic.title}</h1>

          <div className="flex flex-wrap gap-3 sm:gap-4 text-sm text-wabi-muted">
          ...
          {comic.category && (
            <div className="flex items-center gap-2">
              ...
              <Link to={`/search?category=${comic.category._id}`} className="text-wabi-green font-semibold text-sm hover:underline" id="detail-category-link">{comic.category.name}</Link>
            </div>
          )}
          ...
          {comic.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {comic.tags.map((tag) => (
                <Link key={tag} to={`/search?tags=${tag}`} className="badge badge-new text-xs hover:bg-wabi-green/20 transition-colors">#{tag}</Link>
              ))}
            </div>
          )}
          ...
          {comic.rating?.count > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex">
                {[1, 2, 3, 4, 5].map((s) => (
                  <IconStar key={s} className={`w-4 h-4 ${s <= Math.round(comic.rating.avg) ? 'text-wabi-gold' : 'text-wabi-muted'}`} filled={s <= Math.round(comic.rating.avg)} />
                ))}
              </div>
              <span className="text-wabi-muted text-sm">{comic.rating.avg.toFixed(1)} ({comic.rating.count} đánh giá)</span>
            </div>
          )}

          {/* Price card */}
          <div className="paper-old rounded-2xl p-4 sm:p-5 space-y-3">
            <div className="flex items-end gap-3 flex-wrap">
              <span className="text-2xl sm:text-3xl font-extrabold text-wabi-red">{formatPrice(finalPrice)}</span>
              {comic.discount > 0 && <span className="text-wabi-muted text-base sm:text-lg line-through">{formatPrice(comic.price)}</span>}
            </div>

            <div className="flex gap-4 sm:gap-6 flex-wrap">
              <div className="flex items-center gap-1.5">
                <IconBox className={`w-3.5 h-3.5 ${isOutOfStock ? 'text-wabi-red' : 'text-wabi-green'}`} />
                <span className="text-sm">Tồn kho: <span className={`font-bold ${isOutOfStock ? 'text-wabi-red' : 'text-wabi-green'}`}>{isOutOfStock ? 'Hết hàng' : `${comic.stock} cuốn`}</span></span>
              </div>
              <div className="flex items-center gap-1.5">
                <IconTrendingUp className="w-3.5 h-3.5 text-wabi-brown" />
                <span className="text-sm">Đã bán: <span className="font-bold text-wabi-brown">{comic.sold?.toLocaleString()}</span></span>
              </div>
            </div>
          ...
          {!isOutOfStock && (
            <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
              ...
              <button onClick={handleAddToCart} className="btn-primary flex-1 justify-center py-3 cursor-pointer" id="detail-add-to-cart-btn">Thêm Vào Giỏ Hàng</button>
            </div>
          )}

          {comic.description && (
            <div className="paper-old rounded-2xl p-4 sm:p-5">
